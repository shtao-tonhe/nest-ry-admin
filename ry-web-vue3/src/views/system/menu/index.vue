          ...
          <el-col :span="12" v-if="form.menuType == 'F'">
            <el-form-item prop="perms">
              ...
              <template #label>
                <span>
                  <el-tooltip
                    content="控制器中定义的权限字符，如：@RequirePermission('system:user:list')"
                    placement="top"
                  >
                    <el-icon><question-filled /></el-icon>
                  </el-tooltip>
...
import { DICT_TYPE, MENU_TYPE, getDictOptions } from '@/utils/tools/dict'
import { addMenu, delMenu, getMenu, listMenu, updateMenu } from '@/api/system/menu'
import IconSelect from '@/components/System/IconSelect/index.vue'
import { MenuForm } from '@/types/system/menu'
import { parseDateTime } from '@/utils/plugins/dayjs'
import { handleTree } from '@/utils/common'

const { proxy } = getCurrentInstance() as any
const sys_show_hide = getDictOptions(DICT_TYPE.sys_show_hide)
const sys_normal_disable = getDictOptions(DICT_TYPE.sys_normal_disable)

const menuList = ref([]) // 菜单列表
const open = ref(false) // 弹出框控制
const loading = ref(true)
const title = ref('') // 菜单标题
const menuOptions = ref() // 菜单树列表
const isExpandAll = ref(false) // 是否展开全部
const refreshTable = ref(true) // 是否刷新table
const showChooseIcon = ref(false) // 菜单图标选择
const iconSelectRef = ref() // 图标
// 菜单新增表单
let form = ref<MenuForm>({
  parentId: '0',
  menuId: '',
  menuType: 'M',
  icon: '',
...
/** 新增按钮操作 */
async function handleAdd(row: MenuForm | null) {
  await getTreeselect() // 获取菜单树列表
  // 重置内容
  form.value = {
    parentId: '0',
    menuType: 'M',
...
/** 删除按钮操作 */
function handleDelete(row: { menuId: string }) {
  proxy.$modal
    .confirm('确认删除?')
    .then(function () {
